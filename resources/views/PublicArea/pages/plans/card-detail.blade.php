...
@section('title')
Live Football Today | Card Details
@endsection
@section('content')
    <?php
...
@endif
    <div class="payment-method">
        <div class="container">
            <div class="col-md-7 m-auto card_div", Paypal >
                <div class="form-img">
                    <img src="{{ url('PublicArea/img/logo.png') }}" alt="">
                </div>
                <h2 class="payment_text">LiveFootballToday</h2>
                ...
                <form id="payment-form" action="" method="POST">
                    ...
                    @csrf
                    <input type="hidden" name="payment_with" value="card" id="payment_with">
                    <input type="hidden" name="plan_id" value="{{$plan_id}}" id="plan_id">
                    {{--
                    <div id="card-data" class="d-none">
                        ...
                        <div class="checkboxes__row">
                            <div class="checkboxes__item">
                                <label class="checkbox style-e">
                                    ...
                                    <div class="checkbox__body">Monthly Membership
                            ...
                            <div class="checkboxes__item">
                                <label class="checkbox style-e">
                                    ...
                                    <div class="checkbox__checkmark"></div>
                                    <div class="checkbox__body">Yearly Membership (£{{ $membershipprice->yearly_price }})
                                    </div>
                                </label>
                            </div>
                        </div>
                    ...
                    <div class="col-md-12 field-wrap">
                        <fieldset>
                            <legend>Automatic Subscription</legend>
                            <div class="checkboxes__row">
                                <div class="checkboxes__item">
                                    <label class="checkbox style-e">
                                        <input type="radio" name="plan_type" value="automatic" checked="checked"/>
                                        <div class="checkbox__checkmark"></div>
                                ...
                                <div class="checkboxes__item">
                                    <label class="checkbox style-e">
                                        ...
                                        <input type="radio" name="plan_type" value="manual" />
                                        <div class="checkbox__checkmark"></div>
                                        <div class="checkbox__body">No</div>
                                    </label>
                                </div>
                            </div>
                        </fieldset>
                    ...
                    <div class="payment_button">
                        <button type="button" class="" id="paySubmit">Pay With<br>Card</button>
                        <button type="button" id="payPalSubmit" class="ladda-button" data-style="expand-right">
                            <span class="ladda-label">Pay with<br>PayPal</span>
                        </button>
                    </div>
                </form>
    ...
    <div class="modal fade" id="cardModal" tabindex="-1" role="dialog" aria-labelledby="cardModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div id="card-data" class="">
                        <div class="col-md-12 field-wrap">
                            <label for="">Name on Card</label>
                            <input type="text" name="card_name" id="card_name" class="form-control" placeholder="Enter your card name" required>
                        </div>
                        <div class="col-md-12 field-wrap">
                            <label for="">Card Number</label>
                            <div id="card-number" class="form-control" required></div>
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                    <button type="button" class="btn btn-primary" id="btn-pay-with-card">Confirm</button>
                </div>
            </div>
        </div>
    </div>
...
$(document).ready(function(){
    ...
    $("#paySubmit").click(function(e){
        ...
        const paySubmit = document.getElementById("btn-pay-with-card");
        ...
        paySubmit.addEventListener("click", async (event) => {
            ...
            try {
                ...
                if (result.error) {
                    ...
                    cardNameInput.setAttribute('name', 'card_name');
                    cardNameInput.setAttribute('value', $("#card_name").val());
                    payForm.appendChild(hiddenInput);
                    payForm.submit();
                }
            } catch (error) {
    ...
    $("#payPalSubmit").click(function(e){
        ...
        btn.hide();
        var data_obj = {};
        data_obj.plan_id = $("#plan_id").val();
        data_obj.interval =  $('input[name="plan_packge"]:checked').val(); // yearly, monthly
        data_obj.payment_type = $('input[name="plan_type"]:checked').val(); // automatic, manual 
        
        
        $.ajax({
            url:"{{route('stripe.paypal.setup.ajax')}}",
            type:"post",
            data:data_obj,
            success:function(response){
                // console.log(response); return;
                if(response.success){
                    var stripe_public = response.data.stripe_public;
                    const stripe = Stripe(stripe_public,{});
                    
                    if(response.data.payment_type == 'manual'){
                        var client_secret = response.data.intent.client_secret;
                        // Redirects away from the client
                        const error = stripe.confirmPayPalPayment(client_secret,{
                                return_url: "{{route('stripe.paypal.response')}}",
                            }
                        );
                        ...
                        if(response.data.intent.client_secret){
                            
                            var client_secret = response.data.intent.client_secret
                            
                          
                            // Redirects away from the client
                            const error = stripe.confirmPayPalSetup(client_secret,{
                                return_url: "{{route('stripe.paypal.response')}}",
                                mandate_data: {
                                  customer_acceptance: {
                                    type: 'online',
                            ...
                            alert("something went wrong with creating setup intent");
                        }   
                    }
                }else{
...
@section('css')
    ...
    <style>
        ...
        .card_div #paySubmit,.card_div  #payPalSubmit,
        #agreeBtn {
            background: #FFD23F;
            color: #000;
            font-size: 18px;
        ...
        .card_div #paySubmit:hover, .card_div  #payPalSubmit:hover,
        #agreeBtn:hover {
